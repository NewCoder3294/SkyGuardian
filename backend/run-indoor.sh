...
exec env \
  ...
  YOLO_DRONE_WEIGHTS="$MODELS/drone-yolov8x.pt" \
  YOLO_DRONE_KEEP="drone" \
  YOLO_DRONE_CONF="0.15" \
  YOLO_CONF="0.30" \
  YOLO_IMGSZ="640" \
  DEPTH_MODEL="off" \
  PERCEPTION_FPS="5" \
  ...
  UPLOAD_YOLO_DRONE_WEIGHTS="$MODELS/drone-yolov8x.pt" \
  UPLOAD_YOLO_DRONE_KEEP="drone" \
  UPLOAD_YOLO_DRONE_CONF="0.25" \
