...
def test_route_arming_transfers_lock_between_modes():
    ...
    _route_arming_for_command(Command.APPROACH, lock); assert lock.holder == "approach"
    _route_arming_for_command(Command.RECALL, lock); assert lock.holder == "follow"
    _route_arming_for_command(Command.APPROACH, lock); assert lock.holder == "approach"
    _route_arming_for_command(Command.STOP, lock); assert lock.holder == "follow"
