...
export default function DataPage() {
  ...
  return (
    <div className="hud-grid min-h-screen bg-bg text-text">
...
function AskData({
  ...
  const submit = (text: string) => {
    ...
    if (!trimmed) return;
    setAnswer(answerData(trimmed, missions, classes));
  };
      ...
      <div className="px-4 py-4">
        ...
        <div className="mt-3 flex flex-wrap gap-1.5">
          {EXAMPLE_CHIPS.map((c) => (
            <button
              ...
              className="border border-border-strong bg-surface-elevated px-2 py-1 font-mono text-[10px] uppercase tracking-wide text-text-muted transition hover:border-accent/60 hover:text-accent"
            >
              {c}
            </button>
          ))}
        ...
        {answer && (
          <div className="mt-4 border border-accent/30 bg-accent/10 px-3 py-2.5">
            <div className="mb-1 font-mono text-[9px] uppercase tracking-[0.3em] text-text-dim">
              Readout
            </div>
